...
Mention users by ID only if they're in the current chat or have enabled linking when forwarding messages. The bot must also have seen the user. (We'll discuss this concept on the page about [working with users](users#seen-users).)
