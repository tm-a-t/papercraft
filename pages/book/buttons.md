...
When pressed, the button sends [an update](updates) to the bot.
...
Use a switch-to-inline button when a message should lead the user into [bot inline mode](inline).
...
- Callback game button. Opens an [HTML game](html-games).
- Pay button. Used for [integrated payments](payments).
- Web view button. Opens [bot web interface](mini-apps).
- Login URL button. Used for authorization on sites;
  ...
  The button works similarly to [Log In with Telegram](login-widget) but doesn't require entering
