...
Add [buttons](buttons) when the message should offer a choice or action.
...
A bot can't send messages to a user who [blocked it.](pm#block)
...
files up to 20 MB and upload files up to 50 MB. If you use [a local Bot API server](api-comparison) though, these limits extend up 
to 2 GB.
