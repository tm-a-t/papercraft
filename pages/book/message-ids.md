...
Supergroups are a special kind of group with enhanced features that appear like regular groups in the user interface; we'll cover them in more detail [later in the book.](supergroups)
...
::: info TELEGRAM API
Use [Telegram API](api) if the bot needs to retrieve a message by ID.
