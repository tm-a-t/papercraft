...
If you don't plan for groups, forbid adding your bot to groups [in BotFather settings](botfather).
...
With [guest mode](bot-automation#guest-bots), a user mentions the bot and the bot can send one reply based
...
If your bot needs to personally contact new group users, consider join requests to get PM permission—we will explore [join requests](join-requests) later in the book.

## Configure privacy mode for the messages you need { #privacy }

Keep privacy mode on for bots that react only [to commands](commands).
...
- Messages [mentioning](markup#mention) the bot
- System messages
