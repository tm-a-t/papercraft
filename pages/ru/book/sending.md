...
В отличие от пользователей, бот также может добавлять к своим сообщениям [кнопки](buttons).
...
Бот не может отправить сообщение [пользователю, который его заблокировал.](pm#block)
...
Если вы используете [локальный сервер Bot API](api-comparison), лимиты повышаются до 2 ГБ.
