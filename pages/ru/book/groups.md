...
Разработчик бота может запретить добавление в группы [через настройки BotFather](botfather).
...
Чтобы не засорять чат, бот может автоматически удалять вспомогательные сообщения через определённое время.
...
— мы рассмотрим [запросы на вступление](join-requests) позже в книге.

## Privacy mode и видимые сообщения { #privacy }

Многие боты созданы для того, чтобы реагировать только [на команды](commands).
...
- Сообщения с [упоминанием](markup#mention) бота
- Системные сообщения
