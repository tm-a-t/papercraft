...
- кастомные эмодзи (доступны только для ботов с [платным юзернеймом](usernames)).
...
пересылке сообщений.
Кроме того, бот не сможет сделать это, если он ещё не [«видел»](users#seen-users) пользователя.
