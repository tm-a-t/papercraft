...
Даже если ваш бот задуман только для групп или каналов, приветствие в личных сообщениях всё равно пригодится.
Так люди, которые заинтересуются ботом и откроют его профиль, получат ответ, а не столкнутся с молчанием.
В приветственное сообщение можно вставить ссылку «Добавить в группу» с помощью [глубоких ссылок для групп.](links#deep-links-for-groups)
...
Затем он видит [вступительный текст](botfather#customization) и кнопку «Начать».
...
Диплинки объясняются [в главе «Взаимодействие».](links)
== Other libraries
<HelpNeeded/>
:::
...
Убедитесь, что бот корректно обрабатывает ситуацию, когда пользователь отправляет `/start`
после того, как уже начинал диалог раньше.
:::

::: tip Дополнительный ввод
Чтобы сообщение `/start` содержало дополнительную информацию, используйте [диплинки](links).
:::
...
- пользователь оставил [заявку на вступление](join-requests) к группе или каналу, где бот управляет запросами на вступление;
- пользователь авторизовался через бота на сайте [с Telegram Login Widget.](login-widget)
...
Тогда бот не сможет отправлять ему личные сообщения, пока не будет разблокирован.
