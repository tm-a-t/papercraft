...
Отправляя сообщение, бот может показать кнопки одного из двух типов:
_инлайн-кнопки_ появляются под сообщением, а _клавиатурные кнопки_ показываются рядом с полем ввода.

Оба вида кнопок могут отображаться в несколько рядов с несколькими кнопками в каждом ряду.
Подробнее о расположении кнопок можно прочитать в конце этой страницы.
...
Когда пользователь нажимает на такую кнопку, боту приходит [апдейт](updates). После этого бот может сделать что угодно.

После нажатия бот может показать notification или alert.
Чтобы пользователь заметил, что нажатие обработано, 
боту обычно стоит отредактировать сообщение или как-то ещё заметно отреагировать.
Пока этого фидбека нет, на кнопке крутится анимация загрузки.
...
Кнопка для переключения [в инлайн-режим](inline). Можно создать кнопку,
...
только каналы с юзернеймом или только группы, где он админ.

### Другие инлайн-кнопки

Остальные виды кнопок связаны с более редкими фичами ботов:

- Callback game button — кнопка для открытия [HTML-игры](html-games).
- Pay button — кнопка [для платежей](payments).
- Web view button — кнопка для открытия [веб-интерфейса в боте](mini-apps).
- Login URL button — специальная кнопка для авторизации пользователей на сайте.
  ...
  и [Telegram Login Widget](login-widget), но пользователю не нужно вводить номер телефона 
...
Есть опция `resize_keyboard`, которая определяет, должна ли эта клавиатура подстраиваться по высоте.
Почему-то по умолчанию она выключена, из-за чего кнопки выглядят растянутыми:
...
По умолчанию в группе клавиатурные кнопки видны всем участникам. Но вместо этого можно показать их
только упомянутым пользователям, то есть:
